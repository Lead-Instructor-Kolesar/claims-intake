...
def render_user(
    ...
    """
    needed = _placeholders(template.user_template)
    missing = sorted(
        name for name in needed if name != "document_text" and name not in variables
    )
    if missing:
    ...
    values: dict[str, str] = dict(variables)
    values["document_text"] = escaped
